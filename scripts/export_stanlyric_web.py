...
Do not commit full copyrighted lyrics to a public website unless you have rights to do so.
"""
...
def to_records(df: pd.DataFrame, max_docs: int | None = None, min_lyric_chars: int = 80) -> pd.DataFrame:
    ...
    corpus = pd.DataFrame()
    corpus["doc_id"] = df["doc_id"].astype(str) if "doc_id" in df.columns else [f"song_{i:07d}" for i in range(len(df))]
...
def main() -> None:
    ...
    parser.add_argument("--corpus", required=True, type=Path, help="Prepared StanLyric corpus: parquet/csv/json/pickle.")
    parser.add_argument("--output", type=Path, default=Path("assets/json/stanlyric/stanlyric_web_index.json"))
    parser.add_argument("--metrics", type=Path, default=None, help="Optional metrics file from offline evaluation.")
    parser.add_argument("--metrics-output", type=Path, default=Path("assets/json/stanlyric/stanlyric_eval_metrics.json"))
    parser.add_argument("--max-docs", type=int, default=None, help="Optional limit for quick local testing.")
    parser.add_argument("--min-lyric-chars", type=int, default=80)
    parser.add_argument("--k1", type=float, default=1.5)
    parser.add_argument("--b", type=float, default=0.75)
    parser.add_argument("--epsilon", type=float, default=0.25)
    parser.add_argument("--include-full-lyrics", action="store_true", help="Private/local only. Includes full lyric text in the exported JSON.")
    parser.add_argument("--attribution-url", default="https://huggingface.co/datasets/asigalov61/Lyrics-MIDI-Dataset")
    ...
    payload = {
        "metadata": {
            ...
            "license_note": "Lyrics-MIDI-Dataset is listed as CC-BY-NC-SA 4.0; original lyrics belong to source datasets and creators.",
        },
    ...
    print(f"Wrote {args.output} ({len(docs):,} docs, {index['metadata']['vocabulary_size']:,} terms)")
    ...
    if args.include_full_lyrics:
        print("WARNING: --include-full-lyrics writes lyric text into the public JSON artifact. Use only for private/local development unless redistribution is permitted.")
